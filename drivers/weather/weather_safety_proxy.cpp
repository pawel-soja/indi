...
IPState WeatherSafetyProxy::updateWeather()
{
    ...
    if (ret != IPS_OK)
    {
        if (Safety == WSP_SAFE)
        {
            ...
            if (SofterrorCount > softErrorHysteresisNP[WSP_SOFT_ERROR_MAX].value)
            {
...
{
    char *cmd = ScriptsTP[WSP_SCRIPT].text;
...
IPState WeatherSafetyProxy::executeCurl()
{
    ...
    if (curl_handle)
    {
        curl_easy_setopt(curl_handle, CURLOPT_URL, UrlTP[WSP_URL].text);
        curl_easy_setopt(curl_handle, CURLOPT_WRITEFUNCTION, WSP_WriteCallback);
        curl_easy_setopt(curl_handle, CURLOPT_WRITEDATA, &readBuffer);
        curl_easy_setopt(curl_handle, CURLOPT_USERAGENT, "libcurl-agent/1.0");
        LOGF_DEBUG("Call curl %s", UrlTP[WSP_URL].text);
                    ...
                    if (NewSafety != Safety)
                    {
                        ...
                        else if (NewSafety == WSP_SAFE)
                        {
                            if (SofterrorRecoveryMode == true)
                            {
                                ...
                                if (SofterrorRecoveryCount > softErrorHysteresisNP[WSP_SOFT_ERROR_RECOVERY].value)
                                {
