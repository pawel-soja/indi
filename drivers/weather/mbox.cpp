...
IPState MBox::updateWeather()
{
    ...
    *end = '\0';
    ...
    if (strcmp(result[FIRMWARE].c_str(), FirmwareTP[0].text))
    {
...
bool MBox::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        {
            double prevPressure = CalibrationNP[CAL_PRESSURE].value;
            double prevTemperature = CalibrationNP[CAL_TEMPERATURE].value;
            double prevHumidaty = CalibrationNP[CAL_HUMIDITY].value;
            CalibrationNP.update(values, names, n);
            double targetPressure = CalibrationNP[CAL_PRESSURE].value;
            double targetTemperature = CalibrationNP[CAL_TEMPERATURE].value;
            double targetHumidity = CalibrationNP[CAL_HUMIDITY].value;
