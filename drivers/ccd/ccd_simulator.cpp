...
bool CCDSim::ISNewSwitch(const char * dev, const char * name, ISState * states, char * names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        else if (DirectorySP.isNameMatch(name))
        {
            ...
            if (DirectorySP[INDI_ENABLED].getState() == ISS_ON)
            {
                DIR* dirp = opendir(DirectoryTP[0].text);
                struct dirent * dp;
                std::string d_dir = std::string(DirectoryTP[0].text);
                if (DirectoryTP[0].text[strlen(DirectoryTP[0].text) - 1] != '/')
                    d_dir += "/";
                ...
                if (m_AllFiles.empty())
                {
                    ...
                    LOGF_ERROR("No FITS files found in directory %s", DirectoryTP[0].text);
                    DirectorySP.apply();
                }
                else
                {
                    ...
                    LOGF_INFO("Directory-based images are enabled. Subsequent exposures will be loaded from directory %s", DirectoryTP[0].text);
                }
            }
            else
            {
    ...
    if (!strcmp(propName, "ABS_FOCUS_POSITION"))
    {
        for (ep = nextXMLEle(root, 1); ep != nullptr; ep = nextXMLEle(root, 0))
        {
            ...
            if (!strcmp(name, "FOCUS_ABSOLUTE_POSITION"))
            {
                ...
                // calculate FWHM
                double focus       = FocusSimulationNP[0].value;
                double max         = FocusSimulationNP[1].value;
                double optimalFWHM = FocusSimulationNP[2].value;
