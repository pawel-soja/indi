...
{
    int nbuf;
    SetCCDParams(SimulatorSettingsNP[0].value, SimulatorSettingsNP[1].value, 16, SimulatorSettingsNP[2].value,
                 SimulatorSettingsNP[3].value);
    ...
    //  Kwiq
    maxnoise      = SimulatorSettingsNP[8].value;
    skyglow       = SimulatorSettingsNP[9].value;
    maxval        = SimulatorSettingsNP[4].value;
    bias          = SimulatorSettingsNP[5].value;
    limitingmag   = SimulatorSettingsNP[7].value;
    saturationmag = SimulatorSettingsNP[6].value;
    OAGoffset = SimulatorSettingsNP[10].value; //  An oag is offset this much from center of scope position (arcminutes);
    polarError = SimulatorSettingsNP[11].value;
    polarDrift = SimulatorSettingsNP[12].value;
    rotationCW = SimulatorSettingsNP[13].value;
    //  Kwiq++
    king_gamma = SimulatorSettingsNP[14].value * 0.0174532925;
    king_theta = SimulatorSettingsNP[15].value * 0.0174532925;
    TimeFactor = SimulatorSettingsNP[16].value;
...
int GuideSim::SetTemperature(double temperature)
{
    ...
    if (fabs(temperature - TemperatureNP[0].value) < 0.1)
    {
...
void GuideSim::TimerHit()
{
    ...
    {
        if (fabs(TemperatureRequest - TemperatureNP[0].value) <= 0.5)
        {
        ...
        {
            if (TemperatureRequest < TemperatureNP[0].value)
...
int GuideSim::DrawCcdFrame(INDI::CCDChip * targetChip)
{
    ...
    if (GainNP[0].value > 50)
        ...
        exposure_time /= sqrt(50 - GainNP[0].value);
...
bool GuideSim::ISNewNumber(const char * dev, const char * name, double values[], char * names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (strcmp(name, "SIMULATOR_SETTINGS") == 0)
        {
            ...
            limitingmag   = SimulatorSettingsNP[7].value;
            saturationmag = SimulatorSettingsNP[6].value;
            OAGoffset = SimulatorSettingsNP[10].value;
            polarError = SimulatorSettingsNP[11].value;
            polarDrift = SimulatorSettingsNP[12].value;
            rotationCW = SimulatorSettingsNP[13].value;
            //  Kwiq++
        ...
        if (EqPENP.isNameMatch(name))
        {
            ...
            epochPos.dec = EqPENP[AXIS_DE].value;
...
void GuideSim::addFITSKeywords(fitsfile *fptr, INDI::CCDChip *targetChip)
{
    ...
    int status = 0;
    fits_update_key_dbl(fptr, "Gain", GainNP[0].value, 3, "Gain", &status);
}
