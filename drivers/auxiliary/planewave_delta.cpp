    ...
    for (uint8_t i = 0 ; i < HeaterControlSP.size(); i++)
    {
        ...
        switch (controlMode)
        {
            ...
            case HEATER_CONTROL:
            {
                ...
                double surfaceTemperature = TemperatureNP[i == 0 ? TEMPERATURE_BACKPLATE : TEMPERATURE_SECONDARY].value;
                // Temperature Temperature
                double targetTemperature = HeaterParamNP[i]->np[PARAM_CONTROL].value + TemperatureNP[TEMPERATURE_AMBIENT].value;
                // Get target duty
