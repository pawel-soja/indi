...
bool SkySafari::Connect()
{
    ...
    if (rc)
    {
        skySafariClient->setMount(ActiveDeviceTP[ACTIVE_TELESCOPE].text);
        skySafariClient->setServer(SettingsTP[INDISERVER_HOST].getText(), std::stoi(SettingsTP[INDISERVER_PORT].text));
...
bool SkySafari::startServer()
{
    ...
    serv_socket.sin_port        = htons((unsigned short)atoi(SettingsTP[SKYSAFARI_PORT].text));
