...
bool USBDewpoint::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        if (OutputsNP.isNameMatch(name))
        {
            ...
            OutputsNP.apply();
            setOutput(1, OutputsNP[0].value);
            setOutput(2, OutputsNP[1].value);
            setOutput(3, OutputsNP[2].value);
            readSettings();
            return true;
        }
        if (CalibrationsNP.isNameMatch(name))
        {
            ...
            setCalibrations(CalibrationsNP[0].value, CalibrationsNP[1].value, CalibrationsNP[2].value);
            readSettings();
            return true;
        }
        if (ThresholdsNP.isNameMatch(name))
        {
            ...
            ThresholdsNP.apply();
            setThresholds(ThresholdsNP[0].value, ThresholdsNP[1].value);
            readSettings();
            return true;
        }
