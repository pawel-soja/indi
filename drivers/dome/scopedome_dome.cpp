...
bool ScopeDome::UpdatePosition()
{
    ...
    double az = ((double)rotationCounter * -360.0 / stepsPerTurn) + DomeHomePositionNP[0].value;
    az        = fmod(az, 360.0);
    if (az < 0.0)
    {
        az += 360.0;
...
bool ScopeDome::UpdateSensorStatus()
{
    ...
    EnvironmentSensorsNP[10].setValue(getDewPoint(EnvironmentSensorsNP[8].value, EnvironmentSensorsNP[7].value));
...
void ScopeDome::TimerHit()
{
    ...
    if (status == DOME_HOMING)
    {
        ...
        {
            double azDiff = DomeHomePositionNP[0].value - DomeAbsPosNP[0].value;
            ...
            if (getInputState(IN_HOME) || fabs(azDiff) <= DomeParamNP[0].value)
            {
                // Found home (or close enough)
                LOG_INFO("Home sensor found");
                status   = DOME_READY;
                targetAz = DomeHomePositionNP[0].value;
    ...
    else if (DomeAbsPosNP.getState() == IPS_BUSY)
    {
        if ((currentStatus & STATUS_MOVING) == 0)
        {
            ...
            double azDiff = targetAz - DomeAbsPosNP[0].value;
            ...
            if (!refineMove || fabs(azDiff) <= DomeParamNP[0].value)
            {
                if (refineMove)
...
IPState ScopeDome::MoveAbs(double az)
{
    ...
    targetAz      = az;
    double azDiff = az - DomeAbsPosNP[0].value;
