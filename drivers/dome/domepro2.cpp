...
void DomePro2::TimerHit()
{
    ...
    double currentAz = DomeAbsPosNP[0].value;
    if(getDomeAzPos() && std::abs(currentAz - DomeAbsPosNP[0].value) > DOME_AZ_THRESHOLD)
        DomeAbsPosNP.apply();

    std::string domeStatus = StatusTP[STATUS_DOME].text;
    std::string shutterStatus = StatusTP[STATUS_SHUTTER].text;
    if(getDomeStatus() && getShutterStatus() && (strcmp(domeStatus.c_str(), StatusTP[STATUS_DOME].text) != 0 ||
            strcmp(shutterStatus.c_str(), StatusTP[STATUS_DOME].text) != 0))
    {
...
{
    targetAz = DomeAbsPosNP[0].value + azDiff;
...
bool DomePro2::getDomeAzCoast()
{
    ...
    {
        SettingsNP[SETTINGS_AZ_COAST].setValue(coast * (360 / SettingsNP[SETTINGS_AZ_CPR].value));
        return true;
    }
...
bool DomePro2::getDomeHomeAz()
{
    ...
    {
        SettingsNP[SETTINGS_AZ_HOME].setValue(home * (360 / SettingsNP[SETTINGS_AZ_CPR].value));
        return true;
    }
...
bool DomePro2::getDomeParkAz()
{
    ...
    {
        SettingsNP[SETTINGS_AZ_PARK].setValue(park * (360 / SettingsNP[SETTINGS_AZ_CPR].value));
        return true;
    }
...
bool DomePro2::setDomeParkAz(uint32_t az)
{
    char cmd[DRIVER_LEN] = {0};
    uint32_t steps = static_cast<uint32_t>(az * (SettingsNP[SETTINGS_AZ_CPR].value / 360));
    //    if (park >= SettingsNP[SETTINGS_AZ_CPR].value)
    //    {
    //        char err[DRIVER_LEN] = {0};
...
bool DomePro2::calibrateDomeAz(double az)
{
    char cmd[DRIVER_LEN] = {0};
    uint32_t steps = static_cast<uint32_t>(az * (SettingsNP[SETTINGS_AZ_CPR].value / 360));
    //    if (steps >= SettingsNP[SETTINGS_AZ_CPR].value)
    //    {
    //        char err[DRIVER_LEN] = {0};
...
bool DomePro2::getDomeAzPos()
{
    ...
    {
        DomeAbsPosNP[0].setValue(pos * (360 / SettingsNP[SETTINGS_AZ_CPR].value));
        return true;
    }
...
{
    char cmd[DRIVER_LEN] = {0};
    if (az >= SettingsNP[SETTINGS_AZ_CPR].value)
        return false;
