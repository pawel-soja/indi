...
bool BaaderDome::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        if (CalibrateSP.isNameMatch(name))
        {
            ...
            calibrationStart = DomeAbsPosNP[0].value;
...
void BaaderDome::TimerHit()
{
    ...
    if (DomeAbsPosNP.getState() == IPS_BUSY)
    {
        ...
        {
            double speed = 0;
            if (fabs(targetAz - DomeAbsPosNP[0].value) > SIM_DOME_HI_SPEED)
            ...
            {
                if (targetAz > DomeAbsPosNP[0].value)
                {
                    DomeAbsPosNP[0].value += speed;
                }
                else if (targetAz < DomeAbsPosNP[0].value)
                {
                    DomeAbsPosNP[0].value -= speed;
                }
            }
        ...
        if (fabs(targetAz - DomeAbsPosNP[0].value) < DomeParamNP[0].value)
        {
            ...
            if (status == DOME_CALIBRATING)
            {
                if (calibrationStage == CALIBRATION_STAGE1)
                {
                    ...
                    calibrationTarget2 = DomeAbsPosNP[0].value + 2;
...
{
    LOGF_INFO("Attempting to abort dome motion by stopping at %g", DomeAbsPosNP[0].value);
