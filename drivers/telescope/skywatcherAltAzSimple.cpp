    ...
    switch (TrackState)
    {
        ...
        case SCOPE_TRACKING:
        {
            ...
            if (moving)
            {
                ...
                CurrentTrackingTarget.ra = EqNP[AXIS_RA].value;
                CurrentTrackingTarget.dec = EqNP[AXIS_DE].value;
            }
            else
            {
...
void SkywatcherAltAzSimple::UpdateDetailedMountInformation(bool InformClient)
{
    ...
    if (std::string(BasicMountInfoTP[MOTOR_CONTROL_FIRMWARE_VERSION].text) != std::to_string(MCVersion))
    {
        BasicMountInfoTP[MOTOR_CONTROL_FIRMWARE_VERSION].setText(std::to_string(MCVersion));
        BasicMountInfoHasChanged = true;
    }
    if (std::string(BasicMountInfoTP[MOUNT_CODE].text) != std::to_string(MountCode))
    {
        BasicMountInfoTP[MOUNT_CODE].setText(std::to_string(MountCode));
        BasicMountInfoHasChanged = true;
    }
    if (std::string(BasicMountInfoTP[IS_DC_MOTOR].text) != std::to_string(IsDCMotor))
    {
