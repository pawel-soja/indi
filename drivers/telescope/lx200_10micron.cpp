...
void LX200_10MICRON::getBasicData()
{
    ...
    if (!isSimulation())
    {
        ...
        {
            LOGF_INFO("Tracking frequency is %.1f Hz", TrackFreqNP[0].value);
            TrackFreqNP.apply();
        }
        ...
        LOGF_INFO("RefractionModelTemperature is %0+6.1f degrees C", RefractionModelTemperatureNP[0].value);
...
bool LX200_10MICRON::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        if (strcmp(name, REFRACTION_MODEL_TEMPERATURE) == 0)
        {
            ...
            RefractionModelTemperatureNP.apply();
            LOGF_INFO("RefractionModelTemperature set to %0+6.1f degrees C", RefractionModelTemperatureNP[0].value);
            return true;
        }
        if (strcmp(name, REFRACTION_MODEL_PRESSURE) == 0)
        {
            ...
            RefractionModelPressureNP.apply();
            LOGF_INFO("RefractionModelPressure set to %06.1f hPa", RefractionModelPressureNP[0].value);
            return true;
        }
        if (strcmp(name, MODEL_COUNT) == 0)
        {
            ...
            ModelCountNP.apply();
            LOGF_INFO("ModelCount %d", ModelCountNP[0].value);
            return true;
        }
        ...
        if (strcmp(name, MINIMAL_NEW_ALIGNMENT_POINT) == 0)
        {
            ...
            MiniNewAlpNP.update(values, names, n);
            if (0 != AddSyncPointHere(MiniNewAlpNP[MALP_PRA].value, MiniNewAlpNP[MALP_PDEC].value))
            {
        ...
        if (strcmp(name, NEW_ALIGNMENT_POINT) == 0)
        {
            ...
            if (0 != AddSyncPoint(NewAlpNP[ALP_MRA].value, NewAlpNP[ALP_MDEC].value, NewAlpNP[ALP_MSIDE].value,
                                  NewAlpNP[ALP_PRA].value, NewAlpNP[ALP_PDEC].value, NewAlpNP[ALP_SIDTIME].value))
            {
                LOG_ERROR("AddSyncPoint error");
        ...
        if (strcmp(name, NEW_ALIGNMENT_POINTS) == 0)
        {
            ...
            NewAlignmentPointsNP.apply();
            LOGF_INFO("New unnamed Model now has %d alignment points", NewAlignmentPointsNP[0].value);
            return true;
        }
        if (strcmp(name, "TLE_NUMBER") == 0)
        {
            ...
            TLEtoTrackTP.apply();
            TLEfromDatabaseNP.apply();
            LOGF_INFO("Selected TLE nr %.0f from database", TLEfromDatabaseNP[0].value);
...
bool LX200_10MICRON::ISNewText(const char *dev, const char *name, char *texts[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        if (strcmp(name, NEW_MODEL_NAME) == 0)
        {
            ...
            LOGF_INFO("Model saved with name %s", NewModelNameTP[0].text);
            return true;
        }
        if (strcmp(name, "SAT_TLE_TEXT") == 0)
        {
            ...
            if (0 == SetTLEtoFollow(TLEtoTrackTP[0].text))
            {
                TLEtoTrackTP.setState(IPS_OK);
                TLEfromDatabaseNP.setState(IPS_IDLE);
                TLEtoTrackTP.apply();
                TLEfromDatabaseNP.apply();
                LOGF_INFO("Selected TLE %s", TLEtoTrackTP[0].text);
                return true;
            }
            else
            {
        ...
        if (strcmp(name, "SAT_PASS_WINDOW") == 0)
        {
            ...
            if (0 == CalculateSatTrajectory(SatPassWindowTP[SAT_PASS_WINDOW_START].getText(), SatPassWindowTP[SAT_PASS_WINDOW_END].text))
            {
