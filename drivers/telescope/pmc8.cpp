...
void PMC8::getStartupData()
{
    ...
    double longitude = LocationNP[LOCATION_LONGITUDE].value;
    double latitude = LocationNP[LOCATION_LATITUDE].value;
...
bool PMC8::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (!strcmp(dev, getDeviceName()))
    {
        ...
        if (GuideRateNP.isNameMatch(name))
        {
            ...
            if (set_pmc8_guide_rate(PortFD, GuideRateNP[0].value))
...
bool PMC8::SetTrackMode(uint8_t mode)
{
    ...
    {
        if (set_pmc8_custom_ra_track_rate(PortFD, TrackRateNP[AXIS_RA].value))
            return true;
    }
    else
    {
