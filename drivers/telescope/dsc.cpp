...
bool DSC::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (AxisRangeSP.isNameMatch(name))
        {
            ...
            {
                LOGF_INFO("Axis range is from 0 to %.f", AxisSettingsNP[AXIS1_TICKS].value);
            }
            else
            {
...
bool DSC::ReadScopeStatus()
{
    ...
    if (isSimulation())
    {
        snprintf(response, 16, "%06.f\t%06.f", SimEncoderNP[AXIS1_ENCODER].value, SimEncoderNP[AXIS2_ENCODER].value);
    }
    else
    {
    ...
    {
        if (Axis1Encoder < 0)
            Axis1Encoder += AxisSettingsNP[AXIS1_TICKS].value;

        if (Axis2Encoder < 0)
            Axis2Encoder += AxisSettingsNP[AXIS2_TICKS].value;
    }

    // Calculate reverse values
    double Axis1 = Axis1Encoder;
    if (ReverseSP[AXIS1_ENCODER].getState() == ISS_ON)
        Axis1 = AxisSettingsNP[AXIS1_TICKS].value - Axis1;

#if 0
    if (Axis1Diff < 0)
        Axis1Diff += AxisSettingsNP[AXIS1_TICKS].value;
    else if (Axis1Diff > AxisSettingsNP[AXIS1_TICKS].value)
        Axis1Diff -= AxisSettingsNP[AXIS1_TICKS].value;
#endif

    double Axis2 = Axis2Encoder;
    if (ReverseSP[AXIS2_ENCODER].getState() == ISS_ON)
        Axis2 = AxisSettingsNP[AXIS2_TICKS].value - Axis2;
    ...
    double Axis1Degrees = (Axis1 / AxisSettingsNP[AXIS1_TICKS].value * 360.0) + AxisSettingsNP[AXIS1_DEGREE_OFFSET].value;
    double Axis2Degrees = (Axis2 / AxisSettingsNP[AXIS2_TICKS].value * 360.0) + AxisSettingsNP[AXIS2_DEGREE_OFFSET].value;
