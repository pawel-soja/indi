...
bool LX200Pulsar2::storeScopeLocation()
{
    ...
    {
        LocationNP[0].value = lat;
		double stdLon = (lon < 0 ? 360.0 + lon : lon);
        LocationNP[1].value = stdLon;

		LOGF_DEBUG("Mount Controller Latitude: %g Longitude: %g", LocationNP[LOCATION_LATITUDE].value,
...
//    {
//        LocationNP[0].value = (dd < 0 ? -1 : 1) * (abs(dd) + mm / 60.0);
...
//    {
//        LocationNP[1].value = (dd > 0 ? 360.0 - (dd + mm / 60.0) : -(dd - mm / 60.0));
