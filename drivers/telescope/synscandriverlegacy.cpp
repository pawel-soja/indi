...
void SynscanLegacyDriver::UpdateMountInformation(bool inform_client)
{
    ...
    if (std::string(BasicMountInfoTP[MI_FW_VERSION].text) != HandsetFwVersion)
    {
        BasicMountInfoTP[MI_FW_VERSION].setText(HandsetFwVersion);
        BasicMountInfoHasChanged = true;
    }
    if (std::string(BasicMountInfoTP[MI_MOUNT_CODE].text) != MountCodeStr)
    {
        BasicMountInfoTP[MI_MOUNT_CODE].setText(MountCodeStr);
        BasicMountInfoHasChanged = true;
    }

    if (std::string(BasicMountInfoTP[MI_ALIGN_STATUS].text) != AlignmentStatus)
    {
        BasicMountInfoTP[MI_ALIGN_STATUS].setText(AlignmentStatus);
        BasicMountInfoHasChanged = true;
    }
    if (std::string(BasicMountInfoTP[MI_GOTO_STATUS].text) != GotoStatus)
    {
        BasicMountInfoTP[MI_GOTO_STATUS].setText(GotoStatus);
        BasicMountInfoHasChanged = true;
    }
    if (std::string(BasicMountInfoTP[MI_POINT_STATUS].text) != PointingStatus)
    {
        BasicMountInfoTP[MI_POINT_STATUS].setText(PointingStatus);
        BasicMountInfoHasChanged = true;
    }
    if (std::string(BasicMountInfoTP[MI_TRACK_MODE].text) != TrackingMode)
    {
