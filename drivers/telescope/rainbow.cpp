...
bool Rainbow::sendScopeLocation()
{
    ...
    {
        if (dd > 0)
            LocationNP[1].value = 360.0 - (dd + mm / 60.0);
        else
            LocationNP[1].value = (dd - mm / 60.0) * -1.0;

    }

    LOGF_DEBUG("Mount Controller Latitude: %.3f Longitude: %.3f", LocationNP[LOCATION_LATITUDE].value,
