...
bool SynscanDriver::updateProperties()
{
    ...
    if (isConnected())
    {
        ...
        if (InitPark())
        {
            ...
            SetAxis2ParkDefault(m_isAltAz ? 0 : LocationNP[LOCATION_LATITUDE].value);
        }
        else
        {
            SetAxis1Park(359);
            SetAxis2Park(m_isAltAz ? 0 : LocationNP[LOCATION_LATITUDE].value);
            SetAxis1ParkDefault(359);
            SetAxis2ParkDefault(m_isAltAz ? 0 : LocationNP[LOCATION_LATITUDE].value);
        }
    }
...
bool SynscanDriver::Goto(double ra, double dec)
{
    ...
    if (goto_AltAz && m_isAltAz) // only if enabled to use AltAz goto
    {
        ...
        lnobserver.lng = LocationNP[LOCATION_LONGITUDE].value;
        if (lnobserver.lng > 180)
            lnobserver.lng -= 360;
        lnobserver.lat = LocationNP[LOCATION_LATITUDE].value;
...
bool SynscanDriver::MoveNS(INDI_DIR_NS dir, TelescopeMotionCommand command)
{
    ...
    double customRate = CustomSlewRateNP[AXIS_DE].value;
...
bool SynscanDriver::MoveWE(INDI_DIR_WE dir, TelescopeMotionCommand command)
{
    ...
    uint8_t rate = static_cast<uint8_t>(IUFindOnSwitchIndex(&SlewRateSP)) + 1;
    double customRate = CustomSlewRateNP[AXIS_RA].value;
...
ln_hrz_posn SynscanDriver::getAltAzPosition(double ra, double dec)
{
    ...
    Location.lat = LocationNP[LOCATION_LATITUDE].value;
    Location.lng = LocationNP[LOCATION_LONGITUDE].value;
...
void SynscanDriver::sendStatus()
{
    ...
    if (std::string(StatusTP[MI_GOTO_STATUS].text) != m_MountInfo[MI_GOTO_STATUS])
    {
        StatusTP[MI_GOTO_STATUS].setText(m_MountInfo[MI_GOTO_STATUS]);
        BasicMountInfoHasChanged = true;
    }
    if (std::string(StatusTP[MI_POINT_STATUS].text) != m_MountInfo[MI_POINT_STATUS])
    {
        StatusTP[MI_POINT_STATUS].setText(m_MountInfo[MI_POINT_STATUS]);
        BasicMountInfoHasChanged = true;
    }
    if (std::string(StatusTP[MI_TRACK_MODE].text) != m_MountInfo[MI_TRACK_MODE])
    {
