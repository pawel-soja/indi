...
bool CelestronGPS::initProperties()
{
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    GuideRateNP[AXIS_RA].fill("GUIDE_RATE_WE", "W/E Rate", "%0.2f", 0, 1, 0.1, GuideRateNP[AXIS_RA].value);
    GuideRateNP[AXIS_DE].fill("GUIDE_RATE_NS", "N/S Rate", "%0.2f", 0, 1, 0.1, GuideRateNP[AXIS_DE].value);
    ...
    if (isConnected())
    {
                ...
                fwInfo.celestronTrackMode == CELESTRON_TRACK_MODE::CTM_RADEC)
        {
            ...
            if (driver.get_guide_rate(CELESTRON_AXIS::RA_AXIS, &rate))
            {
                ...
                LOGF_DEBUG("Get Guide Rate: RA %f", GuideRateNP[AXIS_RA].value);
                if (driver.get_guide_rate(CELESTRON_AXIS::DEC_AXIS, &rate))
                {
                    ...
                    GuideRateNP.apply();
                    LOGF_DEBUG("Get Guide Rate: Dec %f", GuideRateNP[AXIS_DE].value);
                }
            }
            else
        ...
        if (checkMinVersion(2.3, "date and time setting"))
        {
            ...
            {
                LocationNP[LOCATION_LATITUDE].value = latitude;
                LocationNP[LOCATION_LONGITUDE].value = longitude;
...
bool CelestronGPS::GotoAzAlt(double az, double alt)
{
    if (isSimulation())
    {
        ...
        observer.lat = LocationNP[LOCATION_LATITUDE].value;
        observer.lng = LocationNP[LOCATION_LONGITUDE].value;
...
bool CelestronGPS::ReadScopeStatus()
{
    ...
    if (HasPierSide())
    {
        ...
        LOGF_DEBUG("latitude %g, sop %c, PierSide %c",
                   LocationNP[LOCATION_LATITUDE].value,
                   sop, psc);
    }
    ...
    {
        // Check position
        double lastPosition = FocusAbsPosNP[0].value;
        ...
        if (pos >= 0)
        {
            ...
            if (fabs(lastPosition - FocusAbsPosNP[0].value) > 1)
                FocusAbsPosNP.apply();
        }
...
bool CelestronGPS::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev && std::string(dev) == getDeviceName())
    {
        ...
        if (strcmp(name, "GUIDE_RATE") == 0)
        {
            ...
            //LOGF_DEBUG("Set Guide Rates (0-1x sidereal): Ra %f, Dec %f", GuideRateNP[AXIS_RA].value, GuideRateNP[AXIS_DE].value);
            //LOGF_DEBUG("Set Guide Rates         (0-255): Ra %i, Dec %i", grRa, grDec);
            LOGF_DEBUG("Set Guide Rates: Ra %f, Dec %f", GuideRateNP[AXIS_RA].value, GuideRateNP[AXIS_DE].value);
...
bool CelestronGPS::ISNewText(const char *dev, const char *name, char **texts, char **names, int n)
{
    ...
    if (dev && std::string(dev) == getDeviceName())
    {
        ...
        if (name && std::string(name) == "PEC_LOAD")
        {
            ...
            LOGF_DEBUG("PEC Set %s", PecFileNameTP[0].text);

            PecData pecData;

            // load from file
            if (!pecData.Load(PecFileNameTP[0].text))
            {
                LOGF_WARN("File %s load failed", PecFileNameTP[0].text);
                return false;
            }
            // save to mount
            if (!pecData.Save(&driver))
            {
                LOGF_WARN("PEC Data file %s save to mount failed", PecFileNameTP[0].text);
                return false;
            }
            LOGF_INFO("PEC Data file %s sent to mount", PecFileNameTP[0].text);
        }
    }
...
IPState CelestronGPS::MoveAbsFocuser(uint32_t targetTicks)
{
    ...
    // implement backlash
    int delta = static_cast<int>(targetTicks - FocusAbsPosNP[0].value);
            ...
            (FocusBacklashNP[0].value > 0 && delta < 0))
    {
        ...
        position -= FocusBacklashNP[0].value;
    }
