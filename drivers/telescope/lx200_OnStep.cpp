...
bool LX200_OnStep::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (MaxSlewRateNP.isNameMatch(name))
        {
            ...
            MaxSlewRateNP[0].value = values[0];
        ...
        if (BacklashNP.isNameMatch(name))
        {
            ...
            if (nset == 2)
            {
                ...
                BacklashNP[0].value = bklshdec;
                BacklashNP[1].value = bklshra;
        ...
        if (ElevationLimitNP.isNameMatch(name))
        {
            ...
            if (nset == 2)
            {
                ...
                ElevationLimitNP[0].value = minAlt;
                ElevationLimitNP[1].value = maxAlt;
    ...
    if (minutesPastMeridianNP.isNameMatch(name))
    {
        ...
        if (nset == 2)
        {
            ...
            minutesPastMeridianNP[0].value = minPMEast;
            minutesPastMeridianNP[1].value = minPMWest;
...
bool LX200_OnStep::ReadScopeStatus()
{
    ...
    BacklashNP[0].value = atof(OSbacklashDEC);
    BacklashNP[1].value = atof(OSbacklashRA);
    BacklashNP.apply();
    ...
    GuideRateNP[0].value = pulseguiderate;
    GuideRateNP[1].value = pulseguiderate;
    GuideRateNP.apply();
    ...
    if (OSMountType == MOUNTTYPE_GEM)   //Doesn't apply to non-GEMs
    {
        ...
        minutesPastMeridianNP[0].value = atof(TempValue); // E
        minutesPastMeridianNP[1].value = atof(TempValue2); //W
        minutesPastMeridianNP.apply();
...
void LX200_OnStep::OSUpdateFocuser()
{
    ...
    if (OSFocuser1)
    {
        ...
        current = FocusAbsPosNP[0].value;
        FocusAbsPosNP.apply();
        LOGF_DEBUG("Current focuser: %d, %d", atoi(value), FocusAbsPosNP[0].value);
        //  :FT#  get status
    ...
    {
        getCommandString(PortFD, value, ":fG#");
        OSFocus2TargNP[0].value = atoi(value);
