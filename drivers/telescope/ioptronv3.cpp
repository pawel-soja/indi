...
void IOptronV3::getStartupData()
{
    ...
    if (driver->getStatus(&scopeInfo))
    {
        ...
        char l[32] = {0}, L[32] = {0};
        fs_sexa(l, LocationNP[LOCATION_LATITUDE].value, 3, 3600);
        fs_sexa(L, LocationNP[LOCATION_LONGITUDE].value, 4, 3600);
    ...
    double parkAZ = LocationNP[LOCATION_LATITUDE].value >= 0 ? 0 : 180;
    double parkAL = LocationNP[LOCATION_LATITUDE].value;
    if (InitPark())
    {
...
bool IOptronV3::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (GuideRateNP.isNameMatch(name))
        {
            ...
            if (driver->setGuideRate(GuideRateNP[RA_AXIS].value, GuideRateNP[DEC_AXIS].value))
...
bool IOptronV3::SetTrackEnabled(bool enabled)
{
    if (enabled)
    {
        ...
        if (TrackModeS[TR_CUSTOM].s == ISS_ON)
            SetTrackRate(TrackRateNP[AXIS_RA].value, TrackRateNP[AXIS_DE].value);
    }
