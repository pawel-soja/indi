...
bool LX200_TeenAstro::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        if (SlewAccuracyNP.isNameMatch(name))
        {
            ...
            if (SlewAccuracyNP[0].value < 3 || SlewAccuracyNP[1].value < 3)
...
bool LX200_TeenAstro::sendScopeLocation()
{
    ...
    if (isSimulation())
    {
        LocationNP[LOCATION_LATITUDE].value = 29.5;  // Kuwait - Jasem's home!
    ...
    {
        if (dd > 0)
            LocationNP[LOCATION_LATITUDE].value = dd + mm / 60.0;
        else
            LocationNP[LOCATION_LATITUDE].value = dd - mm / 60.0;
    }
    ...
    {
        if (dd > 0)
            LocationNP[LOCATION_LONGITUDE].value = 360.0 - (dd + mm / 60.0);
        else
            LocationNP[LOCATION_LONGITUDE].value = (dd - mm / 60.0) * -1.0;
    }
    LOGF_DEBUG("Mount Controller Latitude: %g Longitude: %g", LocationNP[LOCATION_LATITUDE].value, LocationNP[LOCATION_LONGITUDE].value);

    if (getSiteElevation(&elev))
    {
        LocationNP[LOCATION_ELEVATION].value = elev;
    }
    else
    {
