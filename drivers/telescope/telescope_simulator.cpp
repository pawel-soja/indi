    ...
    if (isConnected())
    {
		...
		alignment.longitude = Angle(longitude);
	        ...
	        currentRA = (alignment.lst() - Angle(ParkPositionNP[AXIS_RA].value, Angle::ANGLE_UNITS::HOURS)).Hours();
                currentDEC = ParkPositionNP[AXIS_DE].value;
...
bool ScopeSim::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    ...
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (mountModelNP.isNameMatch(name))
        {
            ...
            mountModelNP.apply();
            alignment.setCorrections(mountModelNP[0].value, mountModelNP[1].value,
                                     mountModelNP[2].value, mountModelNP[3].value,
                                     mountModelNP[4].value, mountModelNP[5].value);
