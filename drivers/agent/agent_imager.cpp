...
void Imager::initiateNextFilter()
{
    ...
    if (group > 0 && image > 0 && group <= maxGroup && image <= maxImage)
    {
        ...
        else if (filterSlot != 0 && FilterSlotNP[0].value != filterSlot)
        {
            ...
            LOGF_DEBUG("Group %d of %d, image %d of %d, filer %d, filter set initiated on %s",
                   group, maxGroup, image, maxImage, (int)FilterSlotNP[0].value, FilterSlotNP.getDeviceName());
        }
        else
        {
            initiateNextCapture();
...
void Imager::initiateNextCapture()
{
    if (isRunning())
    {
        if (group > 0 && image > 0 && group <= maxGroup && image <= maxImage)
        {
            ...
            CCDImageBinNP[0].setValue(CCDImageBinNP[1].value = currentGroup()->binning());
            sendNewNumber(&CCDImageBinNP);
            CCDImageExposureNP[0].setValue(currentGroup()->exposure());
            sendNewNumber(&CCDImageExposureNP);
            CCDUploadSettingsTP[0].setText(ImageNameTP[0].text);
            ...
            LOGF_DEBUG("Group %d of %d, image %d of %d, duration %.1fs, binning %d, capture initiated on %s", group,
                   maxGroup, image, maxImage, CCDImageExposureNP[0].value, (int)CCDImageBinNP[0].value,
...
{
    int group = (int)DownloadNP[0].value;
    int image = (int)DownloadNP[1].value;
    char name[128]={0};
    std::ifstream file;
...
bool Imager::initProperties()
{
    ...
    controlledCCD         = ControlledDeviceTP[0].text;
    controlledFilterWheel = ControlledDeviceTP[1].text;
...
bool Imager::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    ...
    {
        if (std::string{name} == std::string{GroupCountNP.getName()})
        {
        ...
        if (std::string{name} == std::string{DownloadNP.getName()})
        {
...
bool Imager::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    if (Imager::DEVICE_NAME == dev)
    {
        if (std::string{name} == std::string{BatchSP.getName()})
        {
            for (int i = 0; i < n; i++)
            {
...
bool Imager::ISNewText(const char *dev, const char *name, char *texts[], char *names[], int n)
{
    if (Imager::DEVICE_NAME == dev)
    {
        if (std::string{name} == std::string{ControlledDeviceTP.getName()})
        {
            ControlledDeviceTP.update(texts, names, n);
            ControlledDeviceTP.apply();
            strncpy(StatusLP[0].label, ControlledDeviceTP[0].getText(), sizeof(StatusLP[0].label));
            CCDImageExposureNP.setDeviceName(ControlledDeviceTP[0].getText());
            CCDImageBinNP.setDeviceName(ControlledDeviceTP[0].getText());
            strncpy(StatusLP[1].label, ControlledDeviceTP[1].getText(), sizeof(StatusLP[1].label));
            FilterSlotNP.setDeviceName(ControlledDeviceTP[1].getText());
            return true;
        }
        if (std::string{name} == std::string{ImageNameTP.getName()})
        {
