...
bool SestoSenso2::isMotionComplete()
{
    ...
    if (isSimulation())
    {
        int32_t nextPos = FocusAbsPosNP[0].value;
    ...
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        else if (MotorHoldSP.isNameMatch(name))
        {
            ...
            else
            {
                ...
                {
                    LOGF_WARN("Motor hold current set to %.1f: This may be insufficent to hold focus", MotorCurrentNP[MOTOR_CURR_HOLD].value);
                }
            }
...
void SestoSenso2::TimerHit()
{
    ...
    if (rc)
    {
        if (fabs(lastPos - FocusAbsPosNP[0].value) > 0)
        {
            FocusAbsPosNP.apply();
            lastPos = FocusAbsPosNP[0].value;
        }
