...
char Lakeside::DecodeBuffer(char * in_response)
{
    ...
    else
    {
        ...
        if (rc > 0)
        {
            ...
            LOGF_DEBUG("DecodeBuffer: Result (%3.1f)", TemperatureNP[0].value);
    ...
    if (rc > 0)
    {
        ...
        LOGF_DEBUG("DecodeBuffer: Result (%3.2f)", TemperatureKNP[0].value);
...
bool Lakeside::gotoPosition(uint32_t position)
{
    ...
    calc_steps = FocusAbsPosNP[0].value - position;

    // MaxTravelNP[0].value is set by "calibrate" via the control box, & read at connect
    if ( position > FocusMaxPosNP[0].value )
    {
        LOGF_ERROR("Position requested (%ld) is out of bounds between %g and %g", position, FocusAbsPosNP[0].min, FocusMaxPosNP[0].value);
...
{
    return MoveAbsFocuser(dir == FOCUS_INWARD ? FocusAbsPosNP[0].value - ticks : FocusAbsPosNP[0].value + ticks);
}
...
void Lakeside::TimerHit()
{
    ...
    if (FocusAbsPosNP.getState() == IPS_BUSY )
    {
        ...
        else
        {
            ...
            rc = updatePosition();
            FocusAbsPosNP.apply();
            LOGF_INFO("Focuser reached requested position %.f", FocusAbsPosNP[0].value);
        }
    }
    ...
    if (FocusAbsPosNP.getState() == IPS_OK || FocusAbsPosNP.getState() == IPS_IDLE)
    {
        ...
        if (rc && fabs(lastTemperature - TemperatureNP[0].value) > TEMPERATURE_THRESHOLD)
        {
            TemperatureNP.apply();
