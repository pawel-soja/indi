...
void PerfectStar::TimerHit()
{
    ...
    if (FocusAbsPosNP.getState() == IPS_BUSY || FocusRelPosNP.getState() == IPS_BUSY)
    {
        ...
        if (status == PS_HALT && targetPosition == FocusAbsPosNP[0].value)
        {
...
{
    uint32_t finalTicks = FocusAbsPosNP[0].value + (ticks * (dir == FOCUS_INWARD ? -1 : 1));
...
bool PerfectStar::setStatus(PS_STATUS targetStatus)
{
    ...
    if (sim)
    {
        ...
        {
            // Moving in state
            if (targetPosition < FocusAbsPosNP[0].value)
                status = PS_IN;
            else
                // Moving out state
                status = PS_OUT;
