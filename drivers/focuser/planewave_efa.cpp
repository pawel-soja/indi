...
IPState EFA::MoveRelFocuser(FocusDirection dir, uint32_t ticks)
{
    ...
    int targetAbsPosition = FocusAbsPosNP[0].value + (relative * direction * reversed);
...
void EFA::TimerHit()
{
    ...
    if (readTemperature())
    {
        ...
        for (size_t i = 0; i < TemperatureNP.size(); i++)
        {
            ...
            {
                aboveThreshold = true;
                m_LastTemperature[i] = TemperatureNP[i].value;
            }
        }
        ...
        if (FanControlSP[FAN_MANUAL].getState() == ISS_OFF)
        {
            ...
            if (FanControlSP[FAN_AUTOMATIC_ABSOLUTE].getState() == ISS_ON)
            {
                ...
                double min_delta = FanControlNP[FAN_MAX_ABSOLUTE].value - FanControlNP[FAN_DEADZONE].value;
                double max_delta = FanControlNP[FAN_MAX_ABSOLUTE].value + FanControlNP[FAN_DEADZONE].value;

                turnOn = TemperatureNP[TEMPERATURE_PRIMARY].value > max_delta;
                turnOff = TemperatureNP[TEMPERATURE_PRIMARY].value < min_delta;
            }
            else if (FanControlSP[FAN_AUTOMATIC_RELATIVE].getState() == ISS_ON)
            {
                // Temperature delta
                double tDiff = TemperatureNP[TEMPERATURE_PRIMARY].value - TemperatureNP[TEMPERATURE_AMBIENT].value;
                // Adjust delta for deadzone
                double min_delta = FanControlNP[FAN_MAX_RELATIVE].value - FanControlNP[FAN_DEADZONE].value;
                double max_delta = FanControlNP[FAN_MAX_RELATIVE].value + FanControlNP[FAN_DEADZONE].value;
    ...
    {
        m_LastPosition = FocusAbsPosNP[0].value;
        FocusAbsPosNP.apply();
    }
