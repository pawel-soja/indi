...
{
    LOGF_DEBUG("A slope=%.0f", FocusModeANP[0].value);//, FocusModeANP[0].value);
    /*
        ...
        FocusModeANP[0].value = slope * (is_negative==1?-1:1);
        FocusModeANP.apply();
        ...
        FocusModeBNP[0].value = slope * (is_negative==1?-1:1);
        FocusModeBNP.apply();
    */
}
...
bool TCFS::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    ...
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (FocusModeANP.isNameMatch(name))
        {
            ...
            dispatch_command(FLSLOP, FocusModeANP[0].value, MODE_A);
            ...
            dispatch_command(FLSIGN, FocusModeANP[0].value, MODE_A);
            if (read_tcfs(response) == false)
            {
                FocusModeANP.setState(IPS_ALERT);
                FocusModeANP.apply("Error reading TCF-S reply.");
        ...
        if (FocusModeBNP.isNameMatch(name))
        {
            ...
            dispatch_command(FLSLOP, FocusModeBNP[0].value, MODE_B);
            if (read_tcfs(response) == false)
            {
            ...
            dispatch_command(FLSIGN, FocusModeBNP[0].value, MODE_B);
            if (read_tcfs(response) == false)
            {
                FocusModeBNP.setState(IPS_ALERT);
                FocusModeBNP.apply("Error reading TCF-S reply.");
        ...
        if (FocusModeSP.isNameMatch(name))
        {
            ...
            else if (!strcmp(sp->name, "Auto A"))
            {
                if(FocusStartModeSP[0].s == ISS_ON)
                {
                    ...
                    uint32_t startPos = -FocusTemperatureNP[0].value * FocusModeANP[0].value
                                        + FocusModeANP[1].value;
                    LOGF_DEBUG("Autocomp A T=%.1f; m=%f; i=%f; p0=%d;",
                               FocusTemperatureNP[0].value,
                               FocusModeANP[0].value,
                               FocusModeANP[1].value,
                               startPos);
            ...
            else
            {
                ...
                {
                    FocusModeSP.setState(IPS_BUSY);
                    uint32_t startPos = -FocusTemperatureNP[0].value * FocusModeBNP[0].value
                                        + FocusModeBNP[1].value;
                    LOGF_DEBUG("Autocomp B T=%.1f; m=%f; i=%f; p0=%d;",
                               FocusTemperatureNP[0].value,
                               FocusModeBNP[0].value,
                               FocusModeBNP[1].value,
                               startPos);
                    MoveAbsFocuser(startPos);
                }
...
void TCFS::TimerHit()
{
    ...
    if(!isSimulation() && currentMode != MANUAL)
    {
        ...
        for(int i = 0; i < 2; i++)
        {
            ...
            {
                FocusTemperatureNP[0].value = f_temperature;

                if (lastTemperature != FocusTemperatureNP[0].value)
                {
    ...
    if (FocusTemperatureNP.getState() == IPS_OK || FocusTemperatureNP.getState() == IPS_BUSY)
    {
        ...
        if (rc == 1)
        {
            ...
            if (fabs(lastTemperature - FocusTemperatureNP[0].value) > 0.01)
            {
                lastTemperature = FocusTemperatureNP[0].getValue();
                FocusTemperatureNP.apply();
            }
