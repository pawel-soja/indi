...
IPState DeepSkyDadAF3::MoveAbsFocuser(uint32_t targetTicks)
{
    ...
    double bcValue = FocusBacklashNP[0].value;
    int diff = targetTicks - FocusAbsPosNP[0].value;
...
void DeepSkyDadAF3::TimerHit()
{
    ...
    if (rc)
    {
        if (fabs(lastTemperature - TemperatureNP[0].value) >= 0.1 ) //more accurate update
        {
            TemperatureNP.apply();
