...
bool SteelDriveII::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        {
            double factor = TemperatureSettingsNP[TC_FACTOR].value;
            double period = TemperatureSettingsNP[TC_PERIOD].value;
            double delta  = TemperatureSettingsNP[TC_DELTA].value;
            bool rc1 = true, rc2 = true, rc3 = true;
            TemperatureSettingsNP.update(values, names, n);

            if (factor != TemperatureSettingsNP[TC_FACTOR].value)
                rc1 = setParameter("TCOMP_FACTOR", to_string(factor));
            if (period != TemperatureSettingsNP[TC_PERIOD].value)
                rc2 = setParameter("TCOMP_PERIOD", to_string(period));
            if (delta != TemperatureSettingsNP[TC_DELTA].value)
...
void SteelDriveII::TimerHit()
{
    ...
    if (temp0 != TemperatureSensorNP[TEMP_0].value ||
            ...
            tempa != TemperatureSensorNP[TEMP_AVG].value)
    {
