...
bool SmartFocus::initProperties()
{
    ...
    FocusRelPosNP[0].setMax(FocusMaxPosNP[0].value); //MaxPositionNP[0].value;
    FocusRelPosNP[0].setValue(10);
    FocusRelPosNP[0].setStep(1);

    FocusAbsPosNP[0].setMin(0.);
    FocusAbsPosNP[0].setMax(FocusMaxPosNP[0].value); //MaxPositionNP[0].value;
...
IPState SmartFocus::MoveAbsFocuser(uint32_t targetPosition)
{
    ...
    else
    {
        ...
        {
            const int error = MotionErrorNP[0].value; // The NGF-S overshoots motions by 3 steps.
