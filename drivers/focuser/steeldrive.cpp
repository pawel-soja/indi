...
bool SteelDrive::initProperties()
{
    ...
    simPosition           = FocusAbsPosNP[0].value;
...
bool SteelDrive::setTemperatureCompensation()
{
    ...
    double coeff      = TemperatureSettingNP[FOCUS_T_COEFF].value;
...
void SteelDrive::TimerHit()
{
    ...
    if (rc)
    {
        if (fabs(lastPos - FocusAbsPosNP[0].value) > STEELDIVE_POSITION_THRESHOLD)
        {
            FocusAbsPosNP.apply();
    ...
    if (FocusTimerNP.getState() == IPS_BUSY)
    {
        ...
        if (sim)
        {
            ...
            {
                FocusAbsPosNP[0].value -= FocusSpeedNP[0].value;
        ...
        if (FocusAbsPosNP[0].value <= 0 || FocusAbsPosNP[0].value >= FocusAbsPosNP[0].max)
        {
            AbortFocuser();
    ...
    if (FocusAbsPosNP.getState() == IPS_BUSY || FocusRelPosNP.getState() == IPS_BUSY)
    {
        ...
        if (fabs(targetPos - FocusAbsPosNP[0].value) < 5)
        {
