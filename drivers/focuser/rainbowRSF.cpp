...
IPState RainbowRSF::MoveRelFocuser(FocusDirection dir, uint32_t ticks)
{
    ...
    double newPosition = FocusAbsPosNP[0].value + relativeTicks;
...
void RainbowRSF::TimerHit()
{
    ...
    if (rc)
    {
        if (abs(m_LastPosition - FocusAbsPosNP[0].value) > 0)
        {
            FocusAbsPosNP.apply();
            m_LastPosition = FocusAbsPosNP[0].value;

            if (GoHomeSP.getState() == IPS_BUSY && FocusAbsPosNP[0].value == homePosition)
            {
                GoHomeSP.setState(IPS_OK);
    ...
    if (m_TemperatureCounter++ == DRIVER_TEMPERATURE_FREQ)
    {
        ...
        if (rc)
        {
            if (abs(m_LastTemperature - TemperatureNP[0].value) >= 0.05)
            {
                TemperatureNP.apply();
                m_LastTemperature = TemperatureNP[0].value;
            }
        }
        // Reset the counter
