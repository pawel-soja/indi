...
bool USBFocusV3::MoveFocuserUF(FocusDirection dir, unsigned int rticks)
{
    ...
    if ((dir == FOCUS_INWARD) && (rticks > FocusAbsPosNP[0].value))
    {
        ticks = FocusAbsPosNP[0].value;
        LOGF_WARN("Requested %u ticks but inward movement has been limited to %u ticks", rticks, ticks);
    }
    else if ((dir == FOCUS_OUTWARD) && ((FocusAbsPosNP[0].value + rticks) > MaxPositionNP[0].value))
    {
        ticks = MaxPositionNP[0].value - FocusAbsPosNP[0].value;
...
IPState USBFocusV3::MoveAbsFocuser(uint32_t targetTicks)
{
    ...
    ticks = targetPos - FocusAbsPosNP[0].value;
...
IPState USBFocusV3::MoveRelFocuser(FocusDirection dir, uint32_t ticks)
{
    ...
    if ((dir == FOCUS_INWARD) && (ticks > FocusAbsPosNP[0].value))
    {
        aticks = FocusAbsPosNP[0].value;
        DEBUGF(INDI::Logger::DBG_WARNING,
               "Requested %u ticks but relative inward movement has been limited to %u ticks", ticks, aticks);
        ticks = aticks;
    }
    else if ((dir == FOCUS_OUTWARD) && ((FocusAbsPosNP[0].value + ticks) > MaxPositionNP[0].value))
    {
        aticks = MaxPositionNP[0].value - FocusAbsPosNP[0].value;
        DEBUGF(INDI::Logger::DBG_WARNING,
...
void USBFocusV3::TimerHit()
{
    ...
    {
        if (backlashMove && (fabs(backlashTargetPos - FocusAbsPosNP[0].value) < 1))
        {
        ...
        {
            if (fabs(targetPos - FocusAbsPosNP[0].value) < 1)
            {
