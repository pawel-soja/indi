...
bool Integra::initProperties()
{
    ...
    FocusAbsPosNP[0].setMax(MaxPositionNP[0].value);
...
bool Integra::relativeGotoMotor(MotorType type, int32_t relativePosition)
{
    ...
    if (type == MOTOR_FOCUS)
    {
        if (relativePosition > 0)
        {
            if (lastFocuserPosition + relativePosition > MaxPositionNP[MOTOR_FOCUS].value)
            {
                ...
                LOGF_INFO("Focus position change %d clipped to %d to stay at MAX %d",
                          relativePosition, newRelativePosition, MaxPositionNP[MOTOR_FOCUS].value);
    ...
    else if (type == MOTOR_ROTATOR)
    {
        if (relativePosition > 0)
        {
            if (lastRotatorPosition + relativePosition > MaxPositionNP[MOTOR_ROTATOR].value)
            {
                ...
                LOGF_INFO("Rotator position change %d clipped to %d to stay at MAX %d",
                          relativePosition, newRelativePosition, MaxPositionNP[MOTOR_ROTATOR].value);
                relativePosition = newRelativePosition;
            }
        }
        else
...
bool Integra::getPosition(MotorType type)
{
    ...
    if (position != -1e6)
    {
        if (type == MOTOR_FOCUS)
        {
            ...
            {
                auto position_from = (int) FocusAbsPosNP[0].value;
        ...
        else if (type == MOTOR_ROTATOR)
        {
            ...
            {
                auto position_from = (int) RotatorAbsPosNP[0].value;
                int position_to = position;
...
void Integra::TimerHit()
{
            ...
            && timeToReadTemperature <= 0)
    {
        ...
        if (rc)
        {
            ...
            {
                lastTemperature = SensorNP[SENSOR_TEMPERATURE].value;
                SensorNP.apply();
            }
        }
    }
    ...
    if (!haveReadFocusPositionAtLeastOnce || FocusAbsPosNP.getState() == IPS_BUSY || FocusRelPosNP.getState() == IPS_BUSY)
    {
        if ( ! isMotorMoving(MOTOR_FOCUS))
        {
            ...
            if (rc)
            {
                ...
                {
                    lastFocuserPosition = FocusAbsPosNP[0].value;
                    FocusAbsPosNP.apply();
                    FocusRelPosNP.apply();
                    if (haveReadFocusPositionAtLeastOnce)
                    {
    ...
    if (!haveReadRotatorPositionAtLeastOnce  || RotatorAbsPosNP.getState() == IPS_BUSY)
    {
        if ( ! isMotorMoving(MOTOR_ROTATOR))
        {
            ...
            if (rc)
            {
                ...
                {
                    lastRotatorPosition = RotatorAbsPosNP[0].value;
...
bool Integra::getMaxPosition(MotorType type)
{
    ...
    else
    {
        LOGF_WARN("Updated %s motor max position from %d to %d",
                  (type == MOTOR_FOCUS) ? "Focuser" : "Rotator", MaxPositionNP[type].value, position);
        MaxPositionNP[type].setValue(position);
        if (type == MOTOR_FOCUS)
        {
            FocusAbsPosNP[0].setMax(MaxPositionNP[type].value);
        }
        else if (type == MOTOR_ROTATOR)
        {
            RotatorAbsPosNP[0].setMax(MaxPositionNP[type].value);
        }
        else
        {
