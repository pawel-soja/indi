...
void NightCrawler::TimerHit()
{
    ...
    {
        lastVoltage = SensorNP[SENSOR_VOLTAGE].value;
        sensorsUpdated = true;
    }
    ...
    if (rc && FocusAbsPosNP[0].value != lastFocuserPosition)
    {
    ...
    if (rc && RotatorAbsPosNP[0].value != lastRotatorPosition)
    {
    ...
    if (rc && GotoAuxNP[0].value != lastAuxPosition)
    {
        lastAuxPosition = GotoAuxNP[0].value;
        absAuxUpdated = true;
    }
    if (absAuxUpdated)
        GotoAuxNP.apply();
...
IPState NightCrawler::MoveRotator(double angle)
{
    ...
    double a = angle;
    double b = GotoRotatorNP[0].value;
    double d = fabs(a - b);
