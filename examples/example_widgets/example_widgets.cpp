...
Dome::Dome()
    : WidgetEventDispatcher(this)
{
    button.fill("Some Button", "Lab1", MAIN_CONTROL_TAB, IP_RW, ISR_NOFMANY, 0);
    button[0].fill("Click Me", "Lab2", ISS_OFF);

    button[0].onChanged(std::bind([&](INDI::Switch &self){
        fprintf(stderr, "Clicked %d %d!\n", self.state(), button[0].state());
    }, std::ref(button[0])));

    shutter.fill("Shutter Door", "", MAIN_CONTROL_TAB, IP_RW, ISR_1OFMANY, 0);

    shutter[0].fill("Open", "", ISS_ON).onSelected([&]{
        if (rain[0].isState(IPS_ALERT))
        {
            shutter.rollback();
            shutter.apply(IPS_ALERT, "It is raining, cannot open Shutter.");
            return;
         //   throw INDI::WidgetException(IPS_ALERT, "It is raining, cannot open Shutter.");
        }

        shutter.apply(IPS_BUSY, "Shutter is opening...");
        sleep(2);
        shutter.apply(IPS_OK, "Shutter is open.");
    });

    shutter[1].fill("Close", "", ISS_OFF).onSelected([&]{
        shutter.apply(IPS_BUSY, "Shutter is closing...");
        sleep(2);
        shutter.apply(IPS_OK, "Shutter is closed.");
    });

    rain.fill("Rain Alert", "", MAIN_CONTROL_TAB);
    rain.snooping("Rain Detector");

    rain[0].fill("Status", "").onChanged([&]{
        if (rain[0].isState(IPS_ALERT))
        {
            if (shutter[0].isState(ISS_ON))
                closeShutter();
            else
                IDMessage(getDeviceName(), "Rain Alert Detected! Dome is already closed.");
        }
        else if (rain[0].previous().s == IPS_ALERT)
            IDMessage(getDeviceName(), "Rain threat passed. Opening the dome is now safe.");
    });

    
    // rain.snoop("Rain Detector");
}
...
{
    shutter.apply(IPS_BUSY, "Rain Alert! Shutter is closing...");
