...
bool Telescope::saveConfigItems(FILE *fp)
{
    ...
    if (HasLocation() && (LocationNP[LOCATION_LONGITUDE].value != 0 || LocationNP[LOCATION_LATITUDE].value != 0))
        LocationNP.save(fp);
...
void Telescope::NewRaDec(double ra, double dec)
{
    ...
    if (EqNP[AXIS_RA].value != ra || EqNP[AXIS_DE].value != dec || EqNP.getState() != lastEqState)
    {
...
bool Telescope::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    ...
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (ParkPositionNP.isNameMatch(name))
        {
            ...
            if (std::isnan(axis1) == false && std::isnan(axis2) == false)
            {
                ...
                if (rc)
                {
                    ...
                    Axis2ParkPosition = ParkPositionNP[AXIS_DE].value;
                }
        ...
        {
            double preAxis1 = TrackRateNP[AXIS_RA].value, preAxis2 = TrackRateNP[AXIS_DE].value;
            ...
            if (TrackState == SCOPE_TRACKING && !strcmp(IUFindOnSwitch(&TrackModeSP)->name, "TRACK_CUSTOM"))
            {
                ...
                if ( (preAxis1 * TrackRateNP[AXIS_RA].value < 0) || (preAxis2 * TrackRateNP[AXIS_DE].value < 0) )
                {
                ...
                rc = SetTrackRate(TrackRateNP[AXIS_RA].value, TrackRateNP[AXIS_DE].value);
        ...
        if (MovementNSSP.isNameMatch(name))
        {
            ...
            if (current_motion == -1 || (last_ns_motion != -1 && current_motion != last_ns_motion))
            {
                if (MoveNS(last_ns_motion == 0 ? DIRECTION_NORTH : DIRECTION_SOUTH, MOTION_STOP))
                {
                    ...
                    TargetNP[AXIS_RA].setValue(EqNP[AXIS_RA].value);
                    TargetNP[AXIS_DE].setValue(EqNP[AXIS_DE].value);
                    TargetNP.apply();
                }
                else
...
bool Telescope::processLocationInfo(double latitude, double longitude, double elevation)
{
    ...
    if (latitude == LocationNP[LOCATION_LATITUDE].value && longitude == LocationNP[LOCATION_LONGITUDE].value &&
            elevation == LocationNP[LOCATION_ELEVATION].value)
    {
