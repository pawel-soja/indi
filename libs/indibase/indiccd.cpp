...
bool CCD::ISNewText(const char * dev, const char * name, char * texts[], char * names[], int n)
{
    ...
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (ActiveDeviceTP.isNameMatch(name))
        {
            ...
            if (strlen(ActiveDeviceTP[ACTIVE_TELESCOPE].text) > 0)
            {
                IDSnoopDevice(ActiveDeviceTP[ACTIVE_TELESCOPE].getText(), "EQUATORIAL_EOD_COORD");
                IDSnoopDevice(ActiveDeviceTP[ACTIVE_TELESCOPE].getText(), "TELESCOPE_INFO");
                IDSnoopDevice(ActiveDeviceTP[ACTIVE_TELESCOPE].getText(), "GEOGRAPHIC_COORD");
            ...
            if (strlen(ActiveDeviceTP[ACTIVE_ROTATOR].text) > 0)
                IDSnoopDevice(ActiveDeviceTP[ACTIVE_ROTATOR].getText(), "ABS_ROTATOR_ANGLE");
            else
                MPSAS = std::numeric_limits<double>::quiet_NaN();

            // JJ ed 2019-12-10
            if (strlen(ActiveDeviceTP[ACTIVE_FOCUSER].text) > 0)
            {
                IDSnoopDevice(ActiveDeviceTP[ACTIVE_FOCUSER].getText(), "ABS_FOCUS_POSITION");
                IDSnoopDevice(ActiveDeviceTP[ACTIVE_FOCUSER].getText(), "FOCUS_TEMPERATURE");
            }
            ...
            if (strlen(ActiveDeviceTP[ACTIVE_FILTER].text) > 0)
            {
                IDSnoopDevice(ActiveDeviceTP[ACTIVE_FILTER].getText(), "FILTER_SLOT");
                IDSnoopDevice(ActiveDeviceTP[ACTIVE_FILTER].getText(), "FILTER_NAME");
            }
...
bool CCD::ISNewNumber(const char * dev, const char * name, double values[], char * names[], int n)
{
    ...
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (!strcmp(name, "GUIDER_FRAME"))
        {
            ...
            if (UpdateGuiderFrame(GuideCCD.ImageFrameNP[0].value, GuideCCD.ImageFrameNP[1].value,
                                  GuideCCD.ImageFrameNP[2].value, GuideCCD.ImageFrameNP[3].value) == false)
        ...
        if (PrimaryCCD.ImagePixelSizeNP.isNameMatch(name))
        {
            if (PrimaryCCD.ImagePixelSizeNP.update(values, names, n) == 0)
            {
                ...
                SetCCDParams(PrimaryCCD.ImagePixelSizeNP[CCDChip::CCD_MAX_X].value,
                             PrimaryCCD.ImagePixelSizeNP[CCDChip::CCD_MAX_Y].value,
                             PrimaryCCD.getBPP(),
                             PrimaryCCD.ImagePixelSizeNP[CCDChip::CCD_PIXEL_SIZE_X].value,
                             PrimaryCCD.ImagePixelSizeNP[CCDChip::CCD_PIXEL_SIZE_Y].value);
        ...
        if (GuideCCD.ImagePixelSizeNP.isNameMatch(name))
        {
            ...
            SetGuiderParams(GuideCCD.ImagePixelSizeNP[CCDChip::CCD_MAX_X].value,
                            GuideCCD.ImagePixelSizeNP[CCDChip::CCD_MAX_Y].value, GuideCCD.getBPP(),
                            GuideCCD.ImagePixelSizeNP[CCDChip::CCD_PIXEL_SIZE_X].value,
                            GuideCCD.ImagePixelSizeNP[CCDChip::CCD_PIXEL_SIZE_Y].value);
        ...
        if (CCDRotationNP.isNameMatch(name))
        {
            ...
            DEBUGF(Logger::DBG_SESSION, "CCD FOV rotation updated to %g degrees.", CCDRotationNP[0].value);
...
void CCD::addFITSKeywords(fitsfile * fptr, CCDChip * targetChip)
{
    ...
    if (HasCooler() || TemperatureNP.getPermission() == IP_RO)
        fits_update_key_dbl(fptr, "CCD-TEMP", TemperatureNP[0].value, 2, "CCD Temperature (Celsius)", &status);
    ...
    {
        fits_update_key_lng(fptr, "XBAYROFF", atoi(BayerTP[0].text), "X offset of Bayer array", &status);
        fits_update_key_lng(fptr, "YBAYROFF", atoi(BayerTP[1].text), "Y offset of Bayer array", &status);
    ...
    if (targetChip->getFrameType() == CCDChip::LIGHT_FRAME && !std::isnan(J2000RA) && !std::isnan(J2000DE))
    {
        ...
        if (WorldCoordSP[0].getState() == ISS_ON && m_ValidCCDRotation && !std::isnan(effectiveFocalLength))
        {
            ...
            double rotation = 360 - CCDRotationNP[0].value;
            if (rotation > 360)
                rotation -= 360;
    ...
    if (sendData)
    {
        ...
        if (bestFit > 50)
        {
            ...
            if (total > 0)
            {
                ...
                DEBUGF(Logger::DBG_DEBUG, "Guide Star X: %g Y: %g FIT: %g", targetChip->RapidGuideDataNP[0].value,
                       targetChip->RapidGuideDataNP[1].value, targetChip->RapidGuideDataNP[2].value);
            }
            else
            {
                     ...
                     bool saveImage /*, bool useSolver*/)
{
    ...
    if (saveImage)
    {
        ...
        std::string prefix = UploadSettingsTP[UPLOAD_PREFIX].text;
...
bool CCD::saveConfigItems(FILE * fp)
{
    ...
    if (CanSubFrame() && PrimaryCCD.ImageFrameNP[2].value > 0)
