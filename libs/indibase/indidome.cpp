...
bool Dome::ISNewNumber(const char * dev, const char * name, double values[], char * names[], int n)
{
    ...
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        else if (ParkPositionNP.isNameMatch(name))
        {
            ...
            Axis1ParkPosition = ParkPositionNP[AXIS_RA].value;
    ...
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (PresetGotoSP.isNameMatch(name))
        {
            ...
            if (rc == IPS_OK || rc == IPS_BUSY)
            {
                ...
                LOGF_INFO("Moving to Preset %d (%.2f degrees).", index + 1, PresetNP[index].value);
                PresetGotoSP.apply();
                return true;
            }
...
bool Dome::GetTargetAz(double &Az, double &Alt, double &minAz, double &maxAz)
{
    ...
    MountCenter.x = DomeMeasurementsNP[DM_EAST_DISPLACEMENT].value; // Positive to East
    MountCenter.y = DomeMeasurementsNP[DM_NORTH_DISPLACEMENT].value;  // Positive to North
    MountCenter.z = DomeMeasurementsNP[DM_UP_DISPLACEMENT].value;    // Positive Up
    ...
    OpticalCenter(MountCenter, OTASide * DomeMeasurementsNP[DM_OTA_OFFSET].value, observer.lat, hourAngle, OptCenter);

    LOGF_DEBUG("OTA_SIDE: %d", OTASide);
    LOGF_DEBUG("OTA_OFFSET: %g  Lat: %g", DomeMeasurementsNP[DM_OTA_OFFSET].value, observer.lat);
    ...
    if (Intersection(OptCenter, OptVector, DomeMeasurementsNP[DM_DOME_RADIUS].value, mu1, mu2))
    {
...
void Dome::UpdateAutoSync()
{
    if ((m_MountState == IPS_OK || m_MountState == IPS_IDLE) && DomeAbsPosNP.getState() != IPS_BUSY && DomeAutoSyncSP[0].s == ISS_ON)
    {
        ...
        if (fabs(targetAz - DomeAbsPosNP[0].value) > DomeParamNP[0].value)
        {
...
void Dome::SetParkDataType(Dome::DomeParkData type)
{
    ...
    {
        case PARK_NONE:
            strncpy(DomeMotionSP[DOME_CW].label, "Open", MAXINDILABEL);
            strncpy(DomeMotionSP[DOME_CCW].label, "Close", MAXINDILABEL);
            break;
...
IPState Dome::MoveAbs(double az)
{
    ...
    else if (rc == IPS_BUSY)
    {
        ...
        DomeMotionSP.setState(IPS_BUSY);
        DomeMotionSP.reset();
        DomeMotionSP[DOME_CW].setState((az > DomeAbsPosNP[0].value) ? ISS_ON : ISS_OFF);
        DomeMotionSP[DOME_CCW].setState((az < DomeAbsPosNP[0].value) ? ISS_ON : ISS_OFF);
        DomeMotionSP.apply();
