...
void StreamManager::newFrame(const uint8_t * buffer, uint32_t nbytes)
{
    ...
    // N is StreamExposureNP[STREAM_DIVISOR].value
    ++m_frameCountDivider;
    if (
    ...
    if (isStreaming() || isRecording())
    {
        ...
        if (allocatedSize > LimitsNP[LIMITS_BUFFER_MAX].value)
        {
...
void StreamManager::asyncStreamThread()
{
    ...
    double frameW = StreamFrameNP[CCDChip::FRAME_W].value;
    double frameH = StreamFrameNP[CCDChip::FRAME_H].value;
    double frameX = StreamFrameNP[CCDChip::FRAME_X].value;
    double frameY = StreamFrameNP[CCDChip::FRAME_Y].value;
...
{
    if (width != StreamFrameNP[CCDChip::FRAME_W].value || height != StreamFrameNP[CCDChip::FRAME_H].value)
    {
                ...
                (RecordStreamSP[RECORD_FRAME].s == ISS_ON))
        {
            if (!m_isRecording)
            {
                ...
                if (RecordStreamSP[RECORD_TIME].s == ISS_ON)
                    LOGF_INFO("Starting video record (Duration): %g secs.", RecordOptionsNP[0].value);
...
bool StreamManager::ISNewNumber(const char * dev, const char * name, double values[], char * names[], int n)
{
    ...
    if (LimitsNP.isNameMatch(name))
    {
        ...
        m_FPSPreview.setTimeWindow(1000.0 / LimitsNP[LIMITS_PREVIEW_FPS].value);
        m_FPSPreview.reset();
    ...
    if (StreamFrameNP.isNameMatch(name))
    {
        ...
        StreamFrameNP.setState(IPS_OK);
        if (StreamFrameNP[CCDChip::FRAME_X].value + StreamFrameNP[CCDChip::FRAME_W].value > subW)
            StreamFrameNP[CCDChip::FRAME_W].setValue(subW - StreamFrameNP[CCDChip::FRAME_X].value);

        if (StreamFrameNP[CCDChip::FRAME_Y].value + StreamFrameNP[CCDChip::FRAME_H].value > subH)
            StreamFrameNP[CCDChip::FRAME_H].setValue(subH - StreamFrameNP[CCDChip::FRAME_Y].value);

        setSize(StreamFrameNP[CCDChip::FRAME_W].value, StreamFrameNP[CCDChip::FRAME_H].value);
...
#endif
            LOGF_INFO("Starting the video stream with target exposure %.6f s (Max theoritical FPS %.f)", StreamExposureNP[0].value,
                      1 / StreamExposureNP[0].value);
...
{
    *x = StreamFrameNP[CCDChip::FRAME_X].value;
    *y = StreamFrameNP[CCDChip::FRAME_Y].value;
    *w = StreamFrameNP[CCDChip::FRAME_W].value;
    *h = StreamFrameNP[CCDChip::FRAME_H].value;
}
